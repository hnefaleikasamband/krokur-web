...
export class AddSingleBoutComponent implements OnInit {
  ...
  initiateFilteredAthletes() {
    this.filteredAthletes = this.opponentControl.valueChanges.pipe(
  ...
  fillClubsArray() {
    this.athleteService.getClubs().subscribe(
      clubs => {
        this.clubs = clubs;
  ...
  initiateFilteredOpponentClub() {
    this.filteredOpponentClubs = this.clubControl.valueChanges.pipe(
  ...
  initiateFilteredOrganizerClub() {
    this.filteredOrganizerClubs = this.organizerControl.valueChanges.pipe(
      startWith<string | Club>(''),
      map(
        value =>
          typeof value === 'string' ? value : value === null ? '' : value.name
  ...
  filterAthletes(name: string): Athlete[] {
    return this.athletes.filter(
      athlete => athlete.name.toLowerCase().indexOf(name.toLowerCase()) === 0
    );
  }
  ...
  onSubmit() {
    if (this.boutForm.valid) {
      ...
      this.athleteService
        .addBoutToAthlete(this.athlete._id, boutParams)
        .subscribe(
          bout => {
